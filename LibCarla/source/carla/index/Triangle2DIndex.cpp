#include "Triangle2DIndex.h"
#include <iterator>

namespace carla {
namespace index {

Triangle2DIndex::Triangle2DIndex(const std::vector<geom::Triangle2D> &triangles) : _triangles(triangles) {

  std::vector<rt_value> tree_entries;

  for (unsigned int i = 0; i < triangles.size(); i++) {
    const geom::Triangle2D& t = triangles[i];
...
std::vector<geom::Triangle2D> Triangle2DIndex::QueryIntersect(const geom::Vector2D& bounds_min, const geom::Vector2D& bounds_max) const {
