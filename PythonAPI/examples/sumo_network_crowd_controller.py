...
def in_bounds(position):
    return 0 <= position.x <= 2000 and 0 <= position.y <= 2000
...
if __name__ == '__main__':
    ...
    while True:
        ...
        while len(crowd_walkers) < NUM_WALKERS:
            position = carla.Vector2D(random.uniform(0, 2000), random.uniform(0, 2000))
            ...
            if in_bounds(position):
                ...
                actor = world.try_spawn_actor(
                    ...
                    trans)
                if actor:
                    crowd_walkers.append(CrowdWalker(network, actor, 5.0))
                world.wait_for_tick()
        ...
        for (i, crowd_walker) in enumerate(crowd_walkers):
            if crowd_walker is not None:
                crowd_walker.set_velocity(crowd_walker.get_preferred_velocity())
