...
void FCarlaServer::FPimpl::BindActions()
{
  ...
  BIND_SYNC(set_actor_collision_enabled) << [this](
      cr::ActorId ActorId,
      bool bEnabled) -> R<void>
  {
    REQUIRE_CARLA_EPISODE();
    ...
    if (!ActorView.IsValid())
    {
      RESPOND_ERROR("unable to set actor enable collision: actor not found");
    }
    auto RootComponent = Cast<UPrimitiveComponent>(ActorView.GetActor()->GetRootComponent());
    if (RootComponent == nullptr)
    {
      RESPOND_ERROR("unable to set actor enable collision: not supported by actor");
    }
    if (bEnabled)
    {
    ...
    else
    {
      ...
      RootComponent->SetCollisionResponseToChannel(ECollisionChannel::ECC_Pawn, ECollisionResponse::ECR_Overlap);
      RootComponent->SetCollisionResponseToChannel(ECollisionChannel::ECC_PhysicsBody, ECollisionResponse::Overlap);
      /*
